...
Current: **2026.7.1** (first release).

## Versioning & release (decided)

Versioning is **date-based CalVer — `YYYY.M.D`** (the date a release is cut; see
[CHANGELOG](CHANGELOG.md) / [RELEASING](RELEASING.md)). There is **no semver runway**
(no `0.x` / `v1.0.0`); releases are dated. The repo is **public**, and the
update-check banner reads GitHub's anonymous Releases API and works (the one-click
upgrade was verified end-to-end on the VM: 2026.6.27 → 2026.6.29 pulled from GitHub).

**"GA" / the bar below is a qualitative north-star, not a version number** — a dated
release *becomes* the GA release once the bar is met. The semver milestones this file
once used (`0.9`, `1.0`) are retired; the *content* of the bar still stands.
...
## The GA bar — "you can feel the thesis"
...
- **Versioning & updates**: by-version releases, changelog, detect-and-notify update
  path (never auto-upgrade). ✅
- Open-source hygiene: MIT, bilingual README with the real philosophy,
  self-contained green test suite. ✅

### GA punch-list — **all 5 done ✅** (shipped to `main`, verified on VM)
1. ✅ **Decision card — honest scope (option A).** GA sells what is *already true*:
   ...
   producer of grounded cards — the execution-result report card — landed later.)_
...
## Post-GA (deliberately deferred)

- ✅ **Execution-result report card** (done, brought forward) — grounded ✓
...
- **Group collaboration depth** (roundtable / workflow) and proactive prediction —
  present and good-enough; not deepened for GA.

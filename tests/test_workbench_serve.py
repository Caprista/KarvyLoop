...
class TestAC4SecurityAndKLockScan:
    ...
    def test_no_print_headers_or_api_key_in_workbench(self):
        ...
        lines = grep_py(r"print.*(headers|api_key|Authorization|ANTHROPIC_API_KEY)",
    ...
    def test_no_real_key_handling_in_workbench(self):
        ...
        # 真 key 即便在注释里也算泄露 → 不跳注释
        lines = grep_py(r"(sk-ant-|sk-[a-z]+-[A-Za-z0-9]{10,})",
                        ROOT / "karvyloop" / "workbench", skip_comments=False)
    ...
    def test_no_apply_or_courier_in_workbench(self):
        """K4 + K5 复检:workbench/ 仍不**含** apply_ 或 Courier.send(调用,注释提及不算)。"""
        lines = grep_py(r"(apply_deontic\(|domain\.apply_\w+\(|courier\.send\(|Courier\.send\()",
                        ROOT / "karvyloop" / "workbench")

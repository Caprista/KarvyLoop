...
def test_console_warns_on_lan_bind(monkeypatch, capsys):
    ...
    # 注意:cmd_console 是函数内 import uvicorn,monkeypatch 必须 patch
    # `uvicorn.run`(顶层),这样 import 后找到的就是 patch 后的 fake
    import uvicorn
    uvicorn_called = []
    def fake_uvicorn_run(*_a, **_kw):
        uvicorn_called.append((_a, _kw))
        # 不 raise KeyboardInterrupt(让函数自然 return)

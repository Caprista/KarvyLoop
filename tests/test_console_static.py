...
def test_panel_migrated_to_ts_module(panel, glob, old_fn):
    ...
    构建产物 + TS 源都在;旧实现不再留在 app.js。
    T4(docs/83)后面板脚本**不再**首屏常驻:index.html 无标签,由 app.js
    _PANEL_SCRIPTS 注册 + _ensurePanelScript 首开注入(懒加载契约另测)。"""
    ...
    assert f'src="/static/{panel}_panel.js"' not in html, f"{panel}_panel.js 不该再首屏常驻(T4 懒加载)"
    assert f'"/static/{panel}_panel.js"' in app_js, f"app.js _PANEL_SCRIPTS 缺 {panel} 注册"
...
def test_app_js_has_decision_prefs_panel():
    ...
    # T4 后 nav 派发 = openers 表(键即 data-panel 名)+ _openLazyPanel 统一先 ensure
    assert "decision_prefs: () => window.KarvyDecisionPrefs.open()" in app_js
...
def test_code_highlight_wired():
    ...
    T5(docs/83)后不再首屏常驻:render.ts 首次遇到代码块才注入 JS+CSS(_ensureHighlight)。"""
    ...
    # T5 懒加载:index.html 不再压首屏(查真标签,注释里提名字不算);注入点在 render.ts
    html = (STATIC_DIR / "index.html").read_text(encoding="utf-8")
    import re as _re_hl
    assert not _re_hl.search(r'<script[^>]*highlight\.min\.js', html), \
        "highlight.min.js 不该再首屏常驻(T5:render.ts 按需注入)"
    assert not _re_hl.search(r'<link[^>]*highlight-github\.min\.css', html), \
        "highlight-github.min.css 不该再首屏常驻(与 JS 同点注入)"
    assert "_ensureHighlight" in render, "render.ts 缺 highlight 懒加载器"
    assert "vendor/highlight.min.js" in render and "highlight-github.min.css" in render
    assert '"hljs-js"' in render and '"hljs-css"' in render, "注入应带防重复 id"
    # 无代码块零加载(先查有没有 pre code 再 ensure)
    i_guard = render.find("if (!blocks.length) return")
    i_ensure = render.find("_ensureHighlight().then")
    assert 0 < i_guard < i_ensure, "应先判无代码块零加载,再按需注入"
...
# ============================================================================
# 四件 UI 收口(2026-07-16):① CFG-01② 面板新增=首配同款预设选择器
# ② 侧栏三组分组(docs/85 IA§1) ③ T4 面板脚本懒加载 ④ T5 highlight 懒加载(见上 highlight 测)
# ============================================================================
import re as _re2


def test_models_panel_add_uses_guided_picker():
    """CFG-01②:models 面板「新增」区复用首配的 provider 预设选择器(_onbPicker,
    自带「高级/自定义」回退);且面板路径 setDefault:false —— 不抢默认 chat、不误验默认模型。"""
    ts = MODELS_TS.read_text(encoding="utf-8")
    # 新增区走引导式(容器 .models-add-guided + _guidedSetup 注入),不再直贴裸全字段表单
    assert "models-add-guided" in ts, "models 面板新增区缺引导式容器"
    assert "_guidedSetup(addWrap" in ts, "新增区应挂 _onbPicker(经 _guidedSetup)"
    assert "{ setDefault: false }" in ts, "面板路径必须声明 setDefault:false(CFG-01② 坑)"
    ...
    # 构建产物同步(没 npm run build 会在这红)
    built = (STATIC_DIR / "models_panel.js").read_text(encoding="utf-8")
    assert "models-add-guided" in built and "setDefault === false" in built
    i18n = (STATIC_DIR / "i18n.js").read_text(encoding="utf-8")
    assert i18n.count('"onb.saved_no_default"') == 2, "onb.saved_no_default 必须 en+zh 各一份"
...
def test_panel_scripts_lazy_registry():
    """③ T4:通用 _ensurePanelScript(name)—— 注册表齐全、防重复 id、以全局真出现为准、
    失败清缓存可重试 + 人话提示;nav 派发统一先 ensure 再调(全局函数载入后才有)。"""
    app_js = (STATIC_DIR / "app.js").read_text(encoding="utf-8")
    assert "_PANEL_SCRIPTS" in app_js and "function _ensurePanelScript(" in app_js
    for name in _LAZY_PANEL_SCRIPTS:
        assert f'"/static/{name}_panel.js"' in app_js, f"_PANEL_SCRIPTS 缺 {name}"
    # 防重复注入 id + 载入后验全局契约 + 失败清缓存(可重试)+ 人话提示
    assert '"panel-js-" + name' in app_js, "注入 script 应带防重复 id"
    assert "global missing" in app_js, "onload 后必须验 window 全局真出现(不是 onload 就信)"
    assert "_panelScriptLoading[name] = null" in app_js, "加载失败应清缓存(网络恢复可重试)"
    assert 't("panel.load_failed"' in app_js, "加载失败要人话提示(不留静默死按钮)"
    # nav 派发:openers 惰性解引用 + 统一 _openLazyPanel
    assert "_panelOpeners" in app_js and "_openLazyPanel(p, _panelOpeners[p])" in app_js
    # 跨面板直调依赖:domains →「新建角色」直调 KarvyRolesPanel → deps 先载齐
    assert _re2.search(r'domains:.*deps:\s*\["roles"\]', app_js), "domains 应声明 roles 依赖"
    # i18n 提示 en+zh 各一份
    i18n = (STATIC_DIR / "i18n.js").read_text(encoding="utf-8")
    assert i18n.count('"panel.load_failed"') == 2


def test_panel_scripts_removed_from_first_screen():
    """③ T4:index.html 首屏只留必需脚本;懒加载批全部移除;非面板入口(boot gate /
    💰 meter / 👀 demo / 诊断卡 / 环境召回 / 知识提示)都已改走 ensure。"""
    html = (STATIC_DIR / "index.html").read_text(encoding="utf-8")
    for name in _LAZY_PANEL_SCRIPTS:
        # 查真 script 标签(注释里提文件名不算——demo 入口注释就点了名)
        assert f'src="/static/{name}_panel.js"' not in html, f"{name}_panel.js 不该再首屏常驻"
    # 首屏必需的还在(误移=白屏):dom/modal/ui_widgets/unlock/render/i18n/desktop/app
    for keep in ("dom.js", "modal.js", "ui_widgets.js", "unlock_panel.js",
                 "render.js", "i18n.js", "desktop.js", "app.js"):
        assert f'src="/static/{keep}"' in html, f"首屏必需脚本 {keep} 被误移"
    app_js = (STATIC_DIR / "app.js").read_text(encoding="utf-8")
    # boot:models gate 与 tokens meter 改为 ensure 后调(行为不变,只挪出解析热路径)
    assert '_openLazyPanel("models", () => window.KarvyModelsPanel.checkSetupGate' in app_js
    assert '_ensurePanelScript("tokens").then(() => window.KarvyTokens.pollMeter())' in app_js
    # startPolling 对未载入的 KarvyTokens 守空(首拍可能还在载入)
    assert "if (window.KarvyTokens) window.KarvyTokens.pollMeter()" in app_js
    # 👀 demo 第一击由 app.js 接(脚本载入后 demo_panel 自绑的 click 接手,不双开)
    assert 'if (window.KarvyDemoPanel) return' in app_js
    # 诊断卡(onSystemError)/ 环境召回 / 知识提示入口都先 ensure
    assert 'await _ensurePanelScript("diagnose")' in app_js
    assert '_openLazyPanel("skills"' in app_js and '_openLazyPanel("memory"' in app_js

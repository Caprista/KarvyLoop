...
# ---------- 每条 agent 回合 per-turn 署名(Hardy:@ 角色回复被错标"小卡")----------

class TestPerTurnSpeakerPersists:
    def test_agent_speaker_persists_in_history(self):
        """agent 回合的 speaker 必须 per-turn 持久 —— 否则历史重渲只能靠前端单个全局,
        @ 角色的回复会被错标成"小卡"。"""
        push_chat_log_line("user", "@anthropologist 你好", "t1")
        push_chat_log_line("agent", "我是文化人类学家…", "t2", speaker="academic-anthropologist")
        push_chat_log_line("agent", "小卡自己回的", "t3")   # 无 speaker → "" = 小卡
        h = get_chat_history()
        assert h[1]["speaker"] == "academic-anthropologist"   # 角色回合带角色署名
        assert h[2]["speaker"] == ""                           # 小卡回合 = 空(前端显示小卡)

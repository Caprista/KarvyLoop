...
def _bundle_payload(sp: str = "External agent here.") -> dict:
    return {
        "system_prompt": sp,
...
def test_ac1_manifest_is_minimal_rejects_missing_required():
    """AC1: 缺 system_prompt → 拒收。**tools 不再强制**(纯人设 agency-agent 必须能进,
    由 LLM 拆解从人设提炼能力为原子)—— 缺 tools 现在合法。"""
    # 缺 system_prompt → 仍拒
    bad1 = {"tools": [{"name": "x"}]}
    with pytest.raises(ManifestError):
        discover_manifest("claude", bad1)
    # 缺 tools(纯人设)→ **现在接受**(不再拒)
    persona = discover_manifest("claude", {"system_prompt": "你是一位资深市场调研专家"})
    assert persona.is_minimal()
...
# ============ AC3:agent-bundle 有 SOUL.md → COPY;无 USER.md → SYNTHESIZE ============
def test_ac3_plan_skip_and_synthesize_strategy(tmp_path: pathlib.Path):
    """AC3: 异构同源表策略 — agent-bundle 有 soul_files → SOUL=COPY;无 user_files → USER=SYNTHESIZE。"""
    # 准备真 SOUL.md 文件
    soul_file = tmp_path / "SOUL.md"
    soul_file.write_text("I am soul. Honest and direct.", encoding="utf-8")
    payload = {
        "system_prompt": "I am an external agent bundle.",
        "tools": [{"name": "memory"}],
        "soul_files": [str(soul_file)],
        # 无 user_files
    }
    m = discover_manifest("agent-bundle", payload)
...
def test_protocol_invariants_and_parsers():
    ...
    for parser, payload in [
        ...
        (parse_agent_bundle_manifest, _bundle_payload()),

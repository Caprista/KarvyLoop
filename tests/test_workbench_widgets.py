...
class TestAC4KLockedSourceScan:
    ...
    def test_no_apply_deontic_in_workbench(self):
        """K4:工作台只读 — 不得含 `apply_deontic` 或 `domain.apply_*` 调用。

        排除 docstring 注释(用 grep 配合 -v 排除 `#` 开头行 + 跳过 `__pycache__`)。
        """
        result = subprocess.run(
            ["grep", "-rEn", "--include=*.py", "--exclude-dir=__pycache__",
             r"apply_deontic\(|domain\.apply_\w+\(", str(ROOT / "karvyloop" / "workbench")],

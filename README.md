...
>
> **Away from home? (remote access.)** The token link only works on your local network — KarvyLoop deliberately opens no public port. The local-first way to reach your console from anywhere is a private mesh VPN such as [Tailscale](https://tailscale.com) (WireGuard-based, free for personal use): install it on the host machine and your phone, sign both into the same account, then use the host's Tailscale IP in the `karvyloop url` link (still `--host 0.0.0.0` + token). Traffic is end-to-end encrypted between *your* devices — no third-party server sees your data, no router port-forwarding, and your instance still never leaves machines you own. Alternatives: plain WireGuard (self-hosted) or Cloudflare Tunnel (needs a domain). What we recommend **against**: forwarding port 8766 on your router — the console's token auth is not designed to face the open internet alone.
>

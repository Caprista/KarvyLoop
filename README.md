...
- 🖥️ **Your devices form one mesh.** Same-owner devices share a causally-ordered log (hybrid logical clocks, gossip convergence — no central server): a belief learned on your desktop or a skill crystallized there **shows up on your laptop**, deletions carry tombstones instead of resurrecting, and a **My Devices** panel shows the roster — each device's capability fingerprint, presence, and an *informed* remove (if a device is the only one providing a capability, you're told exactly what your mesh would permanently lose before you confirm). Lost a device? The panel is shaped like an account-security page: one list of everything with access, and revoking asks you to **type the device's name** — it takes effect on that device's very next request. Cross-network sync goes through the relay (end-to-end encrypted; the relay only ever forwards ciphertext).
...
- 🏠 **Local-first = your data & decisions stay yours — not "the LLM runs on your box".** The runtime, your memory, skills and instance live on your machine (`~/.karvyloop/`) and are never uploaded; the *models* are yours to pick — rent a cloud API **or** run one locally (Ollama). Local-first here means **data sovereignty**, not zero-cloud. **MIT**-licensed; ships **by version** and never upgrades itself without your click.

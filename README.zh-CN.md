...
**你的 agent 第 40 次跑同一件事,比第 1 次更便宜、验得更严、更像你。这就是整个产品。**
...
大多数 agent 框架优化的是**一次调用** —— 第 40 次调用和第 1 次一个价。KarvyLoop 优化的是调用外面的那个 **loop**:*发现活 → 跑 → 验证 → 结晶 → **你拍板** → 再来*。每转一圈,都在你机器上留下点东西 —— 一份**技能**(写成文字的方法)、一条**决策偏好**(你怎么拍板)、一条**认知**(它对你世界的了解)。价值递增,盯梢递减。
...
1. **loop 原生,不是调用原生。**设计单元不是一次 LLM 调用,而是会重复的自运行循环。重复任务不缓存答案(过期答案是毒药),而是把**方法**结晶成一份可读的 `SKILL.md`,下次拿新数据重跑 —— 常常**零 LLM 调用**。
2. **你坐驾驶座 —— 是结构,不是选项。**全行业冲向最大自主性,KarvyLoop 把 **H2A** 焊死:AI 用*决策卡*提案,你拍板。安静是**挣来的**,不是默认的 —— 某类决策的命中率过了统计门(Wilson 95% 下限 ≥ 0.90,35 例以上),它才会*提出*安静接手这一类;你明确授予,错一次即收回。
3. **镜像随便抄,实例抄不走。**代码 MIT,随便 clone、随便 fork。但在你机器上、由你的使用长出来的技能、偏好、认知,是谁也复制不走的本地资产 —— 也没有任何平台能收走。

**收据,不是形容词**:重复任务走零 token 快脑;三平台真沙箱(bubblewrap / Seatbelt / 受限令牌)+ fail-closed 能力令牌;3,800+ 测试含 25 类对抗攻击向量,跑在 CI;你的 agent 的记忆可看可改,细到 *"哪条认知何时被谁、因何取代"*。
...
- 🖥️ **你的设备连成一张 mesh。** 同主人设备共享一条因果有序的日志(混合逻辑时钟 + gossip 收敛,无中央服务器):台式机上学到的认知、结晶出的技能,**笔记本上就能看到**;删除带墓碑、绝不复活;「我的设备」面板列出花名册 —— 每台设备的能力指纹、在线态,以及**知情删除**(某台设备是某能力的唯一提供者时,删除前明确告诉你 mesh 会永久失去什么,再让你确认)。跨网同步走 relay(端到端加密,relay 只见密文只转发)。

...
设计蓝本：
  - hermes-agent-main/ plugins/model-providers/<name>/__init__.py 的 ProviderProfile dataclass
    （name / api_mode / env_vars / base_url / auth_type / fallback_models）
  - openclaw-main/ packages/llm-core/src/types.ts 的 KnownApi 协议字符串作 join key
  - CLAUDE.md REFERENCES.md:41 定性：clean-room 重写、只借数据模型不抄运行时

**借**（hermes ProviderProfile 抽象思想）：
  ✅ 协议字符串 join key（api_mode 字段）
  ✅ tuple-of-env-vars 兜底链
  ✅ per-vendor profile 文件组织
  ✅ alias map（短名 → 主名）

**不借**（hermes 那种 plugin discovery filesystem dance + $HERMES_HOME 用户插件层 —— 0.1.0 用不上）：
  ❌ filesystem 动态发现
  ❌ 用户插件层
...
# ---- API 模式（协议 join key；openclaw KnownApi 的 KarvyLoop 简化版）----
...
class ProviderProfile:
    ...
    设计原则：
      ...
      2. 密钥零侵入 —— api_key 字段**不存在**；运行时按 env_vars 兜底链查
         os.environ（hermes 的 api_key_env_vars tuple 模式）。
      3. 别名机制 —— aliases 是短名 → 主名（kimi → moonshot，glm → zhipu）。
      4. fallback_models 是离线默认列表（OpenClaw 的 fallback_models 模式）；
    ...
    # 密钥来源：tuple-of-env-vars 兜底链（hermes 模式）

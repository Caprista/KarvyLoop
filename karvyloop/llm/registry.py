...
设计蓝本（hermes + openclaw 一致的 Map registry 模式）：
  - hermes providers/__init__.py:_REGISTRY + _ALIASES
...
**不借**：
  ❌ 文件系统动态发现（hermes 的 _discover_providers importlib dance）
  ❌ sourceId 作用域（openclaw 的插件热卸载）
  ❌ profile 自动扩展 auth registry（hermes 的双注册表）
...
def resolve_api_key(profile: ProviderProfile) -> str:
    ...
    设计：hermes get_anthropic_key() 的 KarvyLoop 简化版。

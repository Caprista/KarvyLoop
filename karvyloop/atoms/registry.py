...
class AtomRegistry:
    ...
    def confirm(self, atom_id: str) -> bool:
        """临时原子挣到正式身份(docs/02 §15.5):provisional → False。返回是否真改了。"""
    ...
    def update(self, atom_id: str, *, prompt: Optional[str] = None,
               kind: Optional[str] = None, tools: Optional[list[str]] = None) -> Optional[AtomSpec]:
        """编辑一个原子镜像的 prompt / kind / tools(None=不改;此前只能删了重建)。不存在返 None。
        改 tools 时按工具目录重算 executable/unresolved(诚实标注)。id 是引用键,不可改(要改名=删+建)。"""
        a = self._atoms.get(atom_id)
        if a is None:
            return None
        changes: dict = {}
        if prompt is not None:
            changes["prompt"] = prompt or ""
        if kind is not None:
            if kind not in ("task", "daemon"):
                raise ValueError(f"kind 必须是 task/daemon,得到 {kind!r}")
            changes["kind"] = kind

var KarvySchedulesPanelBundle = (function(exports) {
  "use strict";
  ...
  async function renderSchedulesPanel() {
    ...
    const parseBtn = el("button", {
      ...
      onclick: async () => {
        ...
        if (r.ok && r.data && r.data.ok) {
          ...
          preview.textContent = t("sched.preview", {
            cron: r.data.cron,
            intent: r.data.intent,
    ...
    mk.appendChild(inp);
    mk.appendChild(btns);
    mk.appendChild(preview);
    body.appendChild(mk);
    const data = await _getJSON("/api/schedules");
    const list = data && data.schedules || [];
    if (!list.length) {
      body.appendChild(el("div", { class: "mgmt-empty", text: t("sched.empty") }));
      return;
    }
    const wrap = el("div", { class: "mgmt-list" });
    for (const s of list) {
      const stBadge = s.last_status === "error" ? el("span", { class: "dpref-badge provisional", title: s.last_error || "", text: "⚠ " + t("sched.err") }) : s.last_status === "ok" ? el("span", { class: "dpref-badge confirmed", text: "✓ " + t("sched.ok") }) : null;
      const actions = el("div", { class: "dpref-actions" });
      actions.appendChild(el("button", {
        class: "dpref-edit",
        text: s.enabled ? t("sched.pause") : t("sched.resume"),
        onclick: async () => {
          await _postJSON("/api/schedule/toggle", { id: s.id, enabled: !s.enabled });
          renderSchedulesPanel();
        }
      }));
      actions.appendChild(el("button", {
        class: "dpref-edit",
        text: t("sched.run_now"),
        onclick: async () => {
          await _postJSON("/api/schedule/run_now", { id: s.id });
          renderSchedulesPanel();
        }
      }));
      actions.appendChild(el("button", {
        ...
        onclick: async () => {
          ...
          await _postJSON("/api/schedule/delete", { id: s.id });
          renderSchedulesPanel();
        }
      }));
      wrap.appendChild(el(
        ...
        el(
          ...
          el(
            "div",
            { class: "mc-name" },
            el("span", { text: "⏰ " + (s.title || s.intent) }),
            s.enabled ? null : " ",
            s.enabled ? null : el("span", { class: "dpref-badge provisional", text: t("sched.paused") }),
            stBadge ? " " : null,
            stBadge
          ),
  ...
  async function open() {
    openMgmtModal(t("sched.title"));
    await renderSchedulesPanel();
  }
  const KarvySchedulesPanel = { open };
  window.KarvySchedulesPanel = KarvySchedulesPanel;
  exports.KarvySchedulesPanel = KarvySchedulesPanel;

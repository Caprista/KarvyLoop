var KarvyRenderBundle = (function(exports) {
  "use strict";
  const decodeCache = {};
...
`;
  ...
  function _i18n() {
    return window.KarvyI18n;
  }
  function _hljs() {
    return window.hljs;
  }
  const md = MarkdownIt({ html: false, linkify: true, breaks: false });
  function _sanitize(html2) {
    return purify.sanitize(html2, { ADD_ATTR: ["target", "rel"] });
  }
  ...
  function appendMarkdown(container, text2, cls) {
    ...
    for (let i = 0; i < pres.length; i++) _wrapWithCopy(pres[i]);
  ...
  function renderEvents(container, events) {
    ...
    if (!hasTools) {
      events.forEach((ev) => renderEvent(container, ev));
      return;
    }
    let lastTextIdx = -1;
    for (let i = events.length - 1; i >= 0; i--) {
      if (events[i].type === "text" && (events[i].text || "").trim()) {
        lastTextIdx = i;
        break;
      }
    }
    ...
    events.forEach((ev, idx) => {
      if (idx === lastTextIdx) finalText = ev;
      else processEvents.push(ev);
    });
  ...
  window.KarvyRender = KarvyRender;
  exports.KarvyRender = KarvyRender;

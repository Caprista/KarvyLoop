...
let pendingMode = false, denseGraphMode = false, lastRecallUrl = "";
dom.window.KarvyDom.getJSON = async (url) => {
  ...
  // docs/69 Q4:时点召回(带 q + as_of query)→ 返回该时点的召回块。记下 URL 供断言 as_of 真传了。
  if (url.startsWith("/api/memory/recall")) {
    lastRecallUrl = url;
    return { ok: true, as_of: 1748000000, block: "[记忆] 我在旧公司上班" };
  }
  return null;
};
...
// docs/69 Q4:时点召回块 —— 「知识库」页有"按时点"查询(日期 + 搜索词 → 拉 /api/memory/recall?as_of=)
const asofBox = libBody.querySelector(".mem-asof");
assert.ok(asofBox, "知识库页应有'按时点'召回区(mem-asof)");
const asofQ = asofBox.querySelector(".mem-asof-q");
const asofDate = asofBox.querySelector(".mem-asof-date");
const asofGo = asofBox.querySelector(".mem-asof-go");
assert.ok(asofQ && asofDate && asofGo, "按时点区应有搜索词输入 + date 输入 + 查询按钮");
// 空日期 → 不打后端(现状不变):只填搜索词、不填日期,点查询 → recall 不被调用、无输出块
lastRecallUrl = "";
asofQ.value = "公司"; asofDate.value = "";
asofGo.click();
await new Promise((r) => setTimeout(r, 10));
assert.equal(lastRecallUrl, "", "日期为空不应调用 /api/memory/recall(现状不变)");
// 填了日期 + 搜索词 → 拉时点召回,URL 必带 as_of(epoch 秒),渲染召回块文本
asofDate.value = "2025-06-01";
asofGo.click();
await new Promise((r) => setTimeout(r, 10));
assert.ok(lastRecallUrl.includes("/api/memory/recall"), "填了时点应调用时点召回端点");
assert.ok(/[?&]as_of=\d+/.test(lastRecallUrl), `召回 URL 应带 as_of=epoch 秒(实际 ${lastRecallUrl})`);
assert.ok(/[?&]q=/.test(lastRecallUrl), "召回 URL 应带 q(搜索词)");
const asofBlock = libBody.querySelector(".mem-asof-out .mem-asof-block");
assert.ok(asofBlock && asofBlock.textContent.includes("我在旧公司上班"), "应渲染该时点的召回块文本");

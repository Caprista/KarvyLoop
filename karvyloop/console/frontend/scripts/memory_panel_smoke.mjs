...
assert.ok(body.querySelector(".paged-search"), "已知列表应有搜索框(pagedList)");
assert.ok([...body.querySelectorAll(".mc-name")].some((n) => n.textContent === "偏好直接"), "已知列表主行应是标题");
assert.ok([...body.querySelectorAll(".mc-del")].length >= 1, "每条知识应有删除按钮");
// 来源显示真实出处(Hardy:别给用户看 fed/ingest 代号):有 URL 的 → 可点链接指向真实来源
const srcLink = body.querySelector(".mc-src-link");

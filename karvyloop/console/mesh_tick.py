"""console/mesh_tick.py — 设备 mesh 持续同步 + 探活(docs/74 §5「lease 心跳实驱」的前置地基)。

命名循 knowledge_tick/promotion_tick 先例。每 MESH_TICK_S 一轮:读花名册,对**非本机、
room+relay_url 齐**的每台对端做一次 mesh_sync_with_peer(E2E 经 relay,拨它的 mesh 房):
...
单机用户(花名册无可拨对端)→ 直接返回,零流量零开销。挂载在 console/app.py lifespan
(relay_url 挂了才起;mesh 是增益不是地基)。
"""
...
async def mesh_tick(app: Any) -> dict:
    """一轮 mesh 同步/探活。返回 {peers, synced, failed}(+可选 reason,诚实说明为何没动)。"""
    ...
    if not peers:                                       # 单机用户:零动作零流量
        return {"peers": 0, "synced": 0, "failed": 0}
    my_id = str(device_fingerprint(sd).get("device_id") or "")
    if not my_id:                                       # 本机还没 relay 身份 → 拨不了,诚实返回
        return {"peers": len(peers), "synced": 0, "failed": 0, "reason": "no_identity"}
    ...
    return {"peers": len(peers), "synced": synced, "failed": failed}

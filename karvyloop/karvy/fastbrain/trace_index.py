...
class TraceIndex:
    ...
    def _append(self, table: str, capacity: int, payload: dict, scope: str) -> TraceRecord:
        ...
        with self._lock:
            # hash-chain(TR-5):prev = 上一条的 hash(空表则 "")
            row = self._conn.execute(
                f"SELECT hash FROM {table} ORDER BY seq DESC LIMIT 1"
            ).fetchone()
            ...
            self._evict_until(table, capacity)
            self._conn.commit()
        return TraceRecord(seq=seq, ts=ts, payload=payload, size_bytes=size_bytes,
                           scope=scope, hash=cur_hash)

    def _evict_until(self, table: str, capacity: int) -> None:
        """覆最旧直到总量 < 90% 容量(防频繁 evict 抖动)。"""
        ...
        while True:
            ...
            if total < threshold:
                return
            # 删最旧一条(单条删,防一次删多 = 单事务开销)
            oldest = self._conn.execute(
                f"SELECT seq FROM {table} ORDER BY seq ASC LIMIT 1"
            ).fetchone()
            if oldest is None:
                return
            self._conn.execute(
    ...
    def verify_chain(self, layer: str = "raw") -> tuple[bool, int]:
        ...
        prev_in_chain: Optional[str] = None  # 上一条**有 hash 的**记录的 hash
        for r in rows:
            seq, ts, pj, scope, prev_hash, stored_hash = r
            if not stored_hash:
                # 旧 db 迁移前写的行无 hash → 不可校验(非篡改),跳过 + 重置链
                prev_in_chain = None
                continue
            # 重算 hash(用本行存的 prev_hash)
            expect = _chain_hash(prev_hash or "", pj, scope or GLOBAL_SCOPE, float(ts))
            if expect != stored_hash:
                return (False, int(seq))  # 本行被篡改(payload/scope/ts 改了)
            # 链接连续性:本行 prev_hash 应等于上一有 hash 行的 hash(首/驱逐/legacy 后不校)
            if prev_in_chain is not None and (prev_hash or "") != prev_in_chain:
                return (False, int(seq))  # 中间被删/插
            prev_in_chain = stored_hash
        return (True, -1)

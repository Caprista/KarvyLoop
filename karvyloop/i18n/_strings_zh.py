...
ZH = {
    ...
    "computer.control_not_enabled": (
        "机器控制未开启:本次对 {tool} 的调用**没有执行**。控制你的真桌面默认关闭 —— 要角色能点、"
        "能键入,得主人先按本次会话打开。什么都没发生;请主人开启,或先做别的。"
    ),
    "computer.high_risk_needs_confirm": (
        "高危桌面动作已扣下等拍板:本次对 {tool} 的调用**没有执行**。它看起来不可逆或涉及凭证"
        "(如往密码框键入、永久删除类组合键),需要主人显式确认。没有自动执行任何东西。"
    ),

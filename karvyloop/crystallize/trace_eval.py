...
def pending_quality_count(trace, satisfaction: SatisfactionStore, *,
                          tasks: Optional[Iterable[str]] = None, cap: Optional[int] = None) -> int:
    """数 Trace 里**待 LLM 质量评**的 run(已确定性评、做对站住、尚未质量评)——`judge_pending_quality`
    会处理的就是这些。**不调 LLM、纯计数**,给自适应节奏判"积压够没够":活跃用户攒够了就提前评,
    不傻等满 24h(否则差技能要污染召回排序最多 24h)。

    `cap`:只需判"≥cap 没",数到 cap 就停(省扫描)。无 cap = 数全部(也有内部去重门)。"""
    if trace is None or satisfaction is None:
        return 0
    try:
        task_ids = list(tasks) if tasks is not None else list(trace.all_tasks())
    ...
    for tid in task_ids:
        ...
        for run in runs:
            ...
            sample = satisfaction.sample_by_ref(ref)
            if sample is None or sample.achievement <= 0.0:
                continue   # 与 judge_pending_quality 同三道门:数的就是它会评的
            n += 1
...
__all__ = ["EVAL_FACT_KIND", "SATISFACTION_KIND", "QUALITY_KIND",
           "evaluate_pending", "judge_pending_quality", "pending_quality_count", "rehydrate"]

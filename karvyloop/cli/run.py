...
def _bootstrap_runtime(
    *,
    ...
    # token 记账是**基建**,不是 console 才有的可选开关(Hardy:"能轻易绕过=不是基建,不在统一网关上")。
    # 把账本绑在**网关构造**上:任何走标准 runtime 起网关的路径(console/CLI/脚本/压测)都自动有账本,
    # gateway.complete 咽喉记的 token 必落进真账本、绕不过去。已注册(如测试注入内存账本)则不覆盖。

...
def system_skills_dir() -> Path:
    """包内**只读**的系统技能/资产目录(`karvyloop/system_skills/`)。

    系统级 skill/atom 模板放这里 —— 随包发版、git 管、**真不可变**(`git pull` 即升级)。
    它不在 `~/.karvyloop/`(用户数据区),所以**清用户数据 / reset 动不到它**(Hardy:系统资产
    要有不被删的区域)。比放 `~/.karvyloop/system/` 更稳:不会被 `rm -rf ~/.karvyloop` 误删、
    且随版本升级。加载器(SkillIndex.rebuild_from_disk)同时扫这里 + 用户目录,带 `source` 标签。
    """
...
def load_skills_dir(
    ...
    *,
    ...
    for p in candidates:
        try:
            # 第三方(untrusted)技能:加载前查完整性锁 —— 内容被改动/损坏(hash 对不上)→ fail-loud 拒载,
            # 绝不把篡改过的第三方代码喂给沙箱。锁里没有(旧导入/未锁)→ 放行(unlocked,不误杀历史)。
            fm, _ = parse_frontmatter(p)
            if (fm.raw or {}).get("trust") == "untrusted":
                from karvyloop.registry.skill_lock import verify_lock
                status, detail = verify_lock(dir_, p.parent.name)
                if status == "mismatch":
                    logger.warning(f"[skill] 拒载被篡改/损坏的第三方技能「{p.parent.name}」:{detail}")
                    continue

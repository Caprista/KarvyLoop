...
⚠️ **i18n 边界(Hardy 点名 2026-07):本模块的正则目前只认中文,是第一个"热路径 NL 识别"
需要多语言适配的模块。** 做英文用户时这里要按 active locale 换整套模式(认知动词/过去锚词/
时间词/未来现在词 + parse_past_ref 的相对时间解析),不能只加英文词进同一表(中英语序/时间表达
差异大)。**约束 = 热路径零 LLM**(不能靠模型判语言/意图),所以方案倾向"**按 locale 装不同正则
表**"(确定性、可测、零 token),而非上小分类器。这套决策会**推广到未来任何热路径 NL 识别**
(不止 as_of);设计考量登记在档案 docs/70。现状:英文用户暂无 → 不预建英文表(拿空需求造脚手架
是反模式),但 i18n 那天此模块是第一站。
"""

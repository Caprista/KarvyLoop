...
INGEST_SYSTEM = (
    ...
    "严格输出 JSON 数组,每个元素 {\"content\": \"<一条事实/偏好>\", \"kind\": \"fact|preference\"};"
...
def _facts_from_list(data: list) -> list[dict]:
    ...
    for item in data:
        if isinstance(item, dict):
            ...
            if c:
                facts.append({"content": c, "kind": str(item.get("kind", "fact"))})
        elif isinstance(item, str) and item.strip():
            facts.append({"content": item.strip(), "kind": "fact"})
    return facts
...
def parse_facts(text: str) -> list[dict]:
    ...
    **JSON 优先且严格**(独立 checker 抓到 HIGH:旧兜底会把 prose / JSON 对象整坨当成一条
    ...
    if isinstance(data, dict):
        ...
        return [{"content": c, "kind": str(data.get("kind", "fact"))}] if c else []
...
async def compile_material(material: str, *, gateway: Any, model_ref: str = "") -> list[dict]:
    ...
    **必须先 resolve_model**:gateway.complete 要的是**已解析的具体 model id**;调用方常传
    ...
    async for ev in gateway.complete(
        ...
        system=SystemPrompt(static=[INGEST_SYSTEM]),
    ):
...
async def ingest_material(
    ...
    *,
    ...
    facts = await compile_material(material, gateway=gateway, model_ref=model_ref)
    written: list = []
    reasons: list = []
    for f in facts:
...
__all__ = ["IngestResult", "INGEST_SYSTEM", "parse_facts", "compile_material", "ingest_material"]
